...
class Converter(object):
    ...
    def parse_options(self, opt, twopass=None, nb_threads=None):
        """
        ...
        if nb_threads is not None:
            optlist.extend(['-threads', str(nb_threads)])
        return optlist

    def convert(self, infile, outfile, options, twopass=False, nb_threads=None, timeout=10):
        """
        ...
        Example Usage::
            fn = os.path.join(THIS_DIR, 'test_data/test1.ogg')
            conv = Converter().convert(fn, '/tmp/output.mkv', {
               'format': 'mkv',
               'audio': { 'codec': 'aac' },
               'video': { 'codec': 'h264' }
            })
