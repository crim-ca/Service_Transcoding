...
class FFMpeg(object):
    ...
    @staticmethod
    def _spawn(cmds):
        LOGGER.debug(u'Spawning ffmpeg with command: ' + ' '.join(cmds))
    ...
    def probe(self, fname, posters_as_video=True):
        ...
        >>> info = FFMpeg().probe(os.path.join(THIS_DIR, 'test_data/test1.ogg'))
        >>> info.format
        MediaFormatInfo(format=ogg, duration=0.00)
    ...
    def convert(self, infile, outfile, opts, timeout=10):
        ...
        while True:
            ...
            try:
                ...
                ret = ret[:e.start] + '?' * (e.end - e.start) + ret[e.end:]
            total_output += ret
            buf += ret
            if '\r' in buf:
    ...
    def thumbnail(self, fname, time, outfile, size=None,
        ...
        Example usage::
            FFMpeg().thumbnail(os.path.join(THIS_DIR, 'test_data/test1.ogg'),
                               5, '/tmp/shot.png', '320x240')
    ...
    def thumbnails(self, fname, option_list):
        ...
        Example Usage::
            FFMpeg().thumbnails(os.path.join(THIS_DIR, 'test_data/test1.ogg'),
                                [(5, '/tmp/shot.png', '320x240'),
        ...
        for thumb in option_list:
            ...
            cmds.extend([
                ...
                '-q:v', str(FFMpeg.DEFAULT_JPEG_QUALITY if len(thumb) < 4 else
                            str(thumb[3])),
            ])
        ...
        try:
            ...
            stderr_data = (stderr_data[:e.start] + '?' * (e.end - e.start) +
                           stderr_data[e.end:])
